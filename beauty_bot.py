...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data == 'back_to_menu':
        await query.message.delete()      # Видаляє попереднє меню з процедурами
        await start(update, context)      # Відправляє головне меню з кнопками
        return

    # ... Далі йде інша обробка кнопок ...

...
async def send_reminder(user_id, procedure, date, time, mode="day"):
    try:
        ...
        print(f"⚠️ Error sending reminder to {user_id}: {e}")

# Функція ініціалізації бази даних 🗃️
def init_db():
    ...
    # Додаємо колонку "note" до bookings, якщо її ще немає ✏️
    try:
...
# Викликаємо функцію ініціалізації бази даних 🛠️
init_db()
...
# Старт планувальника для виконання задач 🗓️
scheduler = BackgroundScheduler()
scheduler.start()
...
def get_price_text():
    ...
    # Формуємо текст для відправки клієнту 💬
    price_text = "💎 Наш прайс-лист:\n"
    for service, price in rows:
        price_text += f"📌 {service}: {price} грн\n"
    price_text += "\nДля запису зателефонуйте або напишіть в Instagram 📲"
...
async def edit_schedule_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Підключення до БД для отримання запланованих днів 🗂️
    conn = sqlite3.connect('appointments.db')
    c = conn.cursor()
...
async def edit_day_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Кнопка для введення часу вручну ✍️ та кнопка для збереження 💾
    ...
    await query.edit_message_text(
        ...
        "🌟 Якщо хочеш додати свій час — натисни на кнопку *Додати вручну* ✍️.",
        reply_markup=InlineKeyboardMarkup(keyboard)
    )


# --- РЕДАГУВАННЯ ЧАСУ ВРУЧНУ --- ✍️
async def edit_day_times(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    day = context.user_data.get('edit_day')  # Отримуємо день з контексту
    custom_time = query.data.split('_')[1]  # Отримуємо доданий час

    # Додаємо час до списку вибраних
    chosen_times = context.user_data.get('chosen_times', [])
    if custom_time not in chosen_times:
        chosen_times.append(custom_time)  # Додаємо новий час
    context.user_data['chosen_times'] = chosen_times

    # Показуємо оновлений список годин 🕓
    keyboard = [
        [InlineKeyboardButton(f"✅ {t}", callback_data=f"settime_{t}") for t in chosen_times]
    ]
    keyboard.append([InlineKeyboardButton("Зберегти", callback_data="save_times")])
    keyboard.append([InlineKeyboardButton("⬅️ Назад", callback_data="edit_day")])

    await query.edit_message_text(
        f"Ти додав час: {custom_time} 🕑\n\n"
        "Натисни на будь-який час для редагування або додавання.",
        reply_markup=InlineKeyboardMarkup(keyboard)
    )
    ...
    async def edit_schedule_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        # Створення кнопок для кожного дня 🔲
        keyboard = [
            [InlineKeyboardButton(f"🗓️ {date} {'✅' if date in scheduled_dates else '➕'}",
    ...
    async def edit_day_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        # Кнопка для додавання часу вручну ✍️
        keyboard.append([InlineKeyboardButton("🖊️ Додати вручну", callback_data="custom_time")])
    ...
    async def delete_day_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        # Беремо дати, які вже видалені 🗑️
        conn = sqlite3.connect('appointments.db')
        c = conn.cursor()
    ...
    async def week_calendar_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        c.execute(
            "SELECT date, time, procedure, name, phone, status FROM bookings "
            "WHERE date IN ({}) ORDER BY date, time".format(','.join(['?'] * len(week_dates))),
            tuple(week_dates)
        )
        rows = c.fetchall()
        conn.close()
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # --- Редагування примітки клієнта --- ✍️
    if query.data.startswith("edit_client_note_"):
        ...
        await query.message.reply_text("✍️ Введіть нову примітку для клієнта: (наприклад, змінити стиль чи побажання)")
        return
    ...
    if query.data.startswith("client_history_"):
        ...
        if not rows:
            await query.message.reply_text("📭 Історія записів порожня. Попробуйте пізніше!")
            return

        # Формуємо історію записів 📜
        history_text = "📚 Історія записів:\n\n"
        for record in rows:
            date, time, procedure, status = record
            history_text += (
                f"📅 {date} 🕒 {time} — {procedure}\n"
                f"📍 Статус: {status}\n\n"
            )
        ...
        buttons = [[InlineKeyboardButton("⬅️ До клієнтської бази", callback_data="client_base")]]
        await query.message.reply_text(history_text, reply_markup=InlineKeyboardMarkup(buttons))
    ...
    async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if query.data.startswith("edit_client_note_"):
            ...
            context.user_data["edit_note_phone"] = phone
            context.user_data["step"] = "edit_client_note"
            await query.message.reply_text("✍️ Введіть нову примітку для клієнта (наприклад, побажання або зміни).")
            return
        ...
        if query.data == "client_base":
            ...
            for name, phone, visits, last_seen in rows:
                ...
                buttons = [
                    [InlineKeyboardButton("👁️ Картка клієнта", callback_data=f"client_card_{phone}")],
                    [InlineKeyboardButton("📝 Редагувати примітку", callback_data=f"edit_client_note_{phone}")]
                ]
                await query.message.reply_text(
                    msg, reply_markup=InlineKeyboardMarkup(buttons), parse_mode="Markdown"
        ...
        if query.data == 'edit_price':
            ...
            # Виводимо повідомлення з переліком послуг
            await query.edit_message_text("Оберіть послугу для зміни ціни:",
                                          reply_markup=InlineKeyboardMarkup(keyboard))
            return
        ...
        if query.data.startswith('note_'):
            ...
            async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
                ...
                if query.data.startswith("edit_client_note_"):
                    ...
                    context.user_data["step"] = "edit_client_note"
                    await query.message.reply_text(
                        "✍️ Введіть нову примітку для клієнта (наприклад, побажання або зміни).")
                    return
                ...
                if query.data.startswith("client_card_"):
                    ...
                    conn = sqlite3.connect('appointments.db')
                    c = conn.cursor()
                    c.execute("SELECT name, first_seen, last_seen, total_visits, notes FROM clients WHERE phone=?",
                              (phone,))
                    row = c.fetchone()
                    conn.close()
                    ...
                    if row:
                        ...
                        buttons = [
                            [InlineKeyboardButton("📝 Редагувати примітку", callback_data=f"edit_client_note_{phone}")],
                            [InlineKeyboardButton("📖 Переглянути історію записів",
                                                  callback_data=f"client_history_{phone}")],
                            [InlineKeyboardButton("⬅️ До клієнтської бази", callback_data="client_base")]
                        ]
                ...
                if query.data.startswith('edit_price_'):
                    ...
                    await query.edit_message_text(
                        f"Введіть нову ціну для:\n*{name}* (зараз: {old_price} грн)", parse_mode="Markdown",
                        reply_markup=InlineKeyboardMarkup(
                            [[InlineKeyboardButton("⬅️ Назад", callback_data="edit_price")]]))
                    context.user_data['step'] = 'update_price'
                    return
                ...
                if query.data == 'check_booking':
                    ...
                    conn = sqlite3.connect('appointments.db')
                    c = conn.cursor()
                    c.execute("SELECT id, procedure, date, time, status, note FROM bookings WHERE user_id=?",
                              (user_id,))
                    rows = c.fetchall()
                    conn.close()
    ...
    async def text_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        def main():
            ...
            app.add_handler(
                MessageHandler(filters.TEXT & ~filters.COMMAND, text_handler))  # Обробка текстових повідомлень
